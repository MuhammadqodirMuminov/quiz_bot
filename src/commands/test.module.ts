...
export class TestModule {
	...
	private async handleName() {
		this.bot.once('message', async msg => {
			...
			if (success) {
				if (text == '/create') {
					this.bot.sendMessage(chatId, ms.test.name, { reply_markup: { remove_keyboard: true } }).then(() => {
						this.createTest();
					});
				} else if (text == '/getAll') {
					...
					await Promise.all(
						tests.map(async test => {
							this.sendImageOrDocument(test, chatId, () => {}, {
								parse_mode: 'Markdown',
								reply_markup: mp.testInlineButton,
								caption: ms.test.card(test.code, test.name, test.count),
							});
						})
					);

					await this.bot.sendMessage(chatId, ms.testHomeMessage, { reply_markup: mp.adminMenu });
				}
			}
		});
	}
	...
	private handleText(code: number) {
		this.bot.once('message', async msg => {
			...
			if (existTest) {
				const success = await testService.update(String(existTest._id), { text });
	...
	private handleImage(code: number) {
		this.bot.once('message', async msg => {
			...
			if (existTest) {
				const success = await testService.update(String(existTest._id), { image: image });
	...
	private handleAnswers(code: number) {
		this.bot.once('message', async msg => {
			...
			if (existTest) {
				const success = await testService.update(String(existTest._id), { answers, count });
	...
	private handleSave(code: number) {
		this.bot.once('message', async msg => {
			...
			const existTest = await testService.getOne({ code: code });
			if (existTest) {
				const success = await testService.update(String(existTest._id), { isPublished: true });
				if (success) {
					if (text == '/save') {
						...
						this.bot.sendMessage(chatId, 'Invalid command', { parse_mode: 'Markdown', reply_markup: mp.adminMenu });
					}
				}
			}
		});
